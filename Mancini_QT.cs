...
namespace Mancini_QT
{
    public class Mancini_QT : Indicator
    {
        bool bDone = false;
        ...
        {
            Name = "Mancini Lines";
            Description = "Mancini Support/Resistance Levels";
        ...
        private async void FetchDataFromAPI()
        {
            try
            {
                ...
                {
                    string price = sr.Replace("(major)", "").Trim().Left(4);
                    if (sr.Contains("(major)"))
                        this.AddLineLevel(Convert.ToDouble(price), sr.Trim(), cMajorSupport, 2, LineStyle.Solid);
                    else
                ...
                foreach (string sr in sb2)
                {
                    string price = sr.Replace("(major)", "").Trim().Left(4);
                    if (sr.Contains("(major)"))
                        this.AddLineLevel(Convert.ToDouble(price), sr.Trim(), cMajorResistance, 2, LineStyle.Solid);
                    else
